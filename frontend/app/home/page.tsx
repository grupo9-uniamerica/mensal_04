...
export default function StudyRoomScheduler() {
  ...
  */
  ...
  const fetchRooms = useCallback(async (): Promise<Room[]> => {
    try {
      const response = await fetch('http://34.39.140.137/api/rooms');
  ...
  const handleAddRoom = async (e: React.FormEvent) => {
    ...
    const handleDeleteRoom = async (roomId: number) => {
      try {
        ...
        const response = await fetch(`http://34.39.140.137/api/rooms/${roomId}`, {
          method: "DELETE",
          headers: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch('http://34.39.140.137/api/reservations', {
        method: 'POST',
